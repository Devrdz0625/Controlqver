<x-action-section style=" padding: 0px 40px; box-shadow: 0px 3px 8px 3px rgba(0, 0, 0); font-size: 20px; width: 80%; margin: 0 auto; ">
    <x-slot name="title"> <br>
      <b>  {{ __('AUTENTIFICACIÓN DE DOS FACTORES') }} </b>
    </x-slot>
        ...
        <div class="mt-5" style="text-align: end;">
            @if (! $this->enabled)
                <x-confirms-password wire:then="enableTwoFactorAuthentication">
                    <x-button type="button" wire:loading.attr="disabled" style="background-color:black;">
                     <b>   {{ __('PERMITIR') }} </b>
                    </x-button>
                </x-confirms-password>
            @else
                @if ($showingRecoveryCodes)
                    <x-confirms-password wire:then="regenerateRecoveryCodes">
                        <x-secondary-button class="me-3" style="background-color:black;">
                            {{ __('Regenerar códigos de recuperación') }}
                        </x-secondary-button>
                    </x-confirms-password>
                @elseif ($showingConfirmation)
                    <x-confirms-password wire:then="confirmTwoFactorAuthentication">
                        <x-button type="button" class="me-3" wire:loading.attr="disabled" style="background-color:black;">
                          <b>  {{ __('CONFIRMAR') }} </b>
                        </x-button>
                    </x-confirms-password>
                @else
                ...
                @if ($showingConfirmation)
                    <x-confirms-password wire:then="disableTwoFactorAuthentication">
                        <x-secondary-button wire:loading.attr="disabled" style="background-color:black;">
                          <b>  {{ __('CANCELAR') }} </b>
                        </x-secondary-button>
                    </x-confirms-password>
                @else
